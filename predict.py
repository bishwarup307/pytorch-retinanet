...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--image-dir",
        type=str,
        help="path to directory containing inference images",
    )
    ...
    parser.add_argument(
        "-b", "--batch-size", type=int, default=1, help="batch size for inference"
    )
    parser.add_argument(
        "--num-workers", type=int, default=0, help="number of multiprocessing workers"
    )
    parser.add_argument(
        "--backbone", type=str, default="resnet-50", help="backbone model arch"
    )
    ...
    if opt.backbone not in valid_backbones:
        raise AttributeError(
            f"unknown backbone. we only support {', '.join(valid_backbones)}"
        )
    ...
    logger.info(
        f"running inference on {len(dataset)} images (batch-size = {opt.batch_size})."
    )
    loader = DataLoader(
        dataset,
    ...
    for i, (batch, filenames) in tqdm(enumerate(loader), total=len(loader)):
        with torch.no_grad():
            img_id, confs, classes, bboxes, _, _ = model(batch[0].float().cuda())
